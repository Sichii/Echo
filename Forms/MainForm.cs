...
public partial class MainForm : Form
{
    ...
    private readonly Task ClientHandlerTask;
    ...
    internal List<Client> SafeIterateClients
    {
        ...
        {
            lock (Clients)
                return Clients.Where(client => !client.Disposed && !client.Disposing)
                    .ToList();
        }
    }
    ...
    internal void RefreshThumbnails()
    {
        try
        {
            lock (Sync)
                ...
                foreach (var thumb in SafeIterateClients.Where(client => client.IsRunning)
    ...
    private void AllVisible(bool skipPrimary = false, List<Client>? skipList = default)
    {
        ...
        var x = screens[current]
            .Bounds.Left - 10;
        var y = screens[current]
            .Bounds.Top + 10;
        var xMax = screens[current]
            .Bounds.Right + 50;
        var yMax = screens[current]
            .Bounds.Bottom + 50;
        ...
        foreach (var client in SafeIterateClients.Except(skipList))
        {
            ...
            if (x + client.CliWidth > xMax)
            {
                ...
                if (y + client.WinHeight > yMax)
                {
                    ...
                    y = screens[current]
                        .Bounds.Top + 25;
                    xMax = screens[current]
                        .Bounds.Right + 50;
                    yMax = screens[current]
                        .Bounds.Bottom + 50;
                }
            }
        }
    ...
    private void Commander(string name)
    {
        ...
        cascader.Add(cmdr, new(x, y));

        //set next client to be to the right of the commander window
        x = screens[current]
            .Bounds.Left + cmdr.CliWidth - 10;
        y = screens[current]
            .Bounds.Top + 15;

        //for the first 2 clients that arent the commander
        foreach (var client in SafeIterateClients.Where(client => client != cmdr)
                     .Take(2))
        {
            //resize it to small (or large if 4k)
            if (screens[current]
    ...
    {
        lock (Clients)
            if (!Clients.Select(cli => cli.ProcessId)
                    .Contains(client.ProcessId))
            {
                Clients.Add(client);
        ...
        while (true)
            try
            {
                ...
                foreach (var proc in Process.GetProcessesByName("Darkages")
                             .Where(proc => !SafeIterateClients.Select(client => client.ProcessId)
                                 .Contains(proc.Id)))
                    //for each module in that process
                    // ReSharper disable once ConstantConditionalAccessQualifier
                    foreach (var mod in proc.Modules?.OfType<ProcessModule>() ?? Enumerable.Empty<ProcessModule>())
                        //if that darkages window contains dawnd.dll
                        if (mod.ModuleName?.Equals("dawnd.dll", StringComparison.CurrentCultureIgnoreCase) ?? false)
                        {
                            //add it
                            ...
                            {
                                if (DateTime.UtcNow.Subtract(now)
                ...
                foreach (var client in SafeIterateClients.Where(c => DateTime.UtcNow.Subtract(c.Creation)
                             .TotalSeconds > 5))
                {
                    client.TrySetVersion();

                    if (client.Version == null || client.Version.CharacterNameAddressValue == 0)
                        continue;
                    ...
                    var _ = client.Pms.Read(buffer, 0, 13);
